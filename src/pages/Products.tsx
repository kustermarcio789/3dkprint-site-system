...
export default function ProductsPage() {
  ...
  // Carregar produtos do localStorage (gerenciados pelo admin)
  useEffect(() => {
  ...
  const categoryCounts = categories.map(cat => ({
    ...
    count: cat.id === 'all' 
      ? products.length
      : products.filter(p => {
          if (cat.id === 'pecas-impressora') return p.category === 'Peças de Impressora 3D';
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesCategory = selectedCategory === 'all' || 
      ...
      (selectedCategory !== 'pecas-impressora' && product.category.toLowerCase() === categories.find(c => c.id === selectedCategory)?.name.toLowerCase());
